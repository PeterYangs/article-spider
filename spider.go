...
type Spider struct {
	...
	CustomDownloadFun   func(imgUrl string, imgPath string, f *Form, item Field, field string) error //自实现图片下载
}
...
func (s *Spider) CustomDownloadImage(fun func(imgUrl string, imgPath string, f *Form, item Field, field string) error) *Spider {
	...
	case Normal, Api:
		...
		if s.form.ChannelFunc == nil {
			...
			for pageCurrent = s.form.PageStart; pageCurrent < s.form.PageStart+s.form.Length; pageCurrent++ {
				...
				//当前列表url
				uu := s.form.Host + strings.Replace(s.form.Channel, "[PAGE]", strconv.Itoa(pageCurrent), -1)

				//放到form中
				s.form.ListUrlCurrent = uu
